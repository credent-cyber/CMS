...
const HelpDesk: React.FC<IBuddyAppProps> = (props: IBuddyAppProps) => {
  ...
  const normalizedCurrentUserRole = normalizeText(currentUserRole);
  const isElevatedUser = normalizedCurrentUserRole.includes("super admin");

  const isLdSiteAdmin = normalizedCurrentUserRole.includes("l&d site admin");
...
//   );
    ...
    return locationGroups
      .map((group) => {
        const groupMatches =
          ...
          group.ldSiteAdmins.some(admin =>
            normalizeText(admin.name).includes(term) ||
            normalizeText(admin.email).includes(term)
          );
  ...
  const isRaisedByMe = useCallback(
    (row: RequestRow) => normalizeText(row.userEmail) === normalizeText(currentUserEmail),
    [currentUserEmail],
  );
  ...
  const loadHelpDeskData = useCallback(async () => {
    try {
      ...
      const helpDeskUsers = await empMasterWeb.lists
        ...
        .filter("IsHelpDeskExecutive eq 1 and substringof('Super Admin', Role) and EmployeeStatus eq 'Active'")
      ...
      const ldAdmins = await empMasterWeb.lists
        ...
        .filter("substringof('L&D Site Admin', Role) and IsHelpDeskExecutive eq 1 and EmployeeStatus eq 'Active'")
        .top(5000)
        .get();
      ...
      ldAdmins.forEach((admin: any) => {
        ...
        locationsMap.get(location)!.ldSiteAdmins.push({
  ...
  const handleSubmitRequest = async () => {
    ...
    // Prevent L&D Admin from creating request to themselves
    if (normalizeText(selectedContact.email) === normalizeText(currentUserEmail)) {
      showSnackbar("You cannot create a request for yourself", "error", 2500);
      return;
    }
          ...
          {filteredGroups.map((group) => (
            <section key={group.location} className={styles.locationCard}>
              ...
              <div className={styles.cardBody}>
                ...
                {group.ldSiteAdmins.length > 0 && (
                  <>
                    <div className={styles.helpDeskTitle} style={{ marginTop: "0.75rem"}}>
                      <AdminPanelSettingsOutlinedIcon sx={{ fontSize: "1rem" }} />
                      L&D Site Admin{group.ldSiteAdmins.length > 1 ? 's' : ''}
                    </div>

                    <div className={styles.helpDeskUsers}>
                      {group.ldSiteAdmins.map((admin, index) => {
                        const isCurrentUser = normalizeText(admin.email) === normalizeText(currentUserEmail);
                        return (
                        <button
                          key={`${group.location}-admin-${index}`}
                          type="button"
                          className={styles.userCard}
                          onClick={() =>
                            openRequestDrawer(
                              { name: admin.name, email: admin.email, location: group.location, globalId: admin.globalId },
                              "admin",
                            )
                          }
                          disabled={isCurrentUser}
                          title={isCurrentUser ? "You cannot create a request for yourself" : ""}
                        ...
                        >
                          <div className={styles.userInfo}>
                            ...
                            <div className={styles.userDetails}>
                              <div className={styles.userName}>{admin.name} {isCurrentUser ? "(You)" : ""}</div>
                              <div className={styles.userEmail}>{admin.email || "Email not available"}</div>
  ...
  const requestColumns: GridColDef[] = useMemo(() => {
    ...
    cols.push({
      ...
      renderCell: (params: GridRenderCellParams<RequestRow>) => {
        ...
        const raisedByMe = isRaisedByMe(row);
        ...
        if ((owned && status === "completed") || raisedByMe) {
          return (
            <Button
              size="small"
              variant="outlined"
    ...
    return cols;
  }, [isElevatedUser, isAssignedToMe, isRaisedByMe]);
