...
const statusChipClass = (status?: string): string => {
  ...
  if (s === "resolved" || s === "closed" || s === "completed") return "statusResolved";
  if (s === "rejected" || s === "cancelled") return "statusRejected";
...
//   );
  ...
  const loadHelpDeskRequests = useCallback(async () => {
    try {
      ...
      // Non-elevated users only see requests they raised themselves.
      if (!isElevatedUser && currentUserEmail) {
        query = query.filter(`UserEmail eq '${escSP(currentUserEmail)}'`);
      }
                  ...
                  <TableRow key={row.id} hover>
                    ...
                    <TableCell>
                      <Chip
                        ...
                        size="small"
                        className={`${styles.statusChipTable} ${styles[statusChipClass(row.status)]}`}
                      />
                    </TableCell>
                  </TableRow>
                ))}
