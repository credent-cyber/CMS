...
//   );
  ...
  // ---- "My Pending Actions" toggle logic ----
  // When ON, restricts the list to Pending requests that belong in the
  // current user's approval bucket:
  //   - L&D Site Admin -> requests raised from their assigned location
  //   - IT HelpDesk     -> requests assigned to their email (HelpdeskEmail)
  //   - Everyone else   -> their own raised requests that are still pending
  // Super Admin never sees this toggle (handled in the UI), so no case needed here.
  const myPendingBucketRows = useMemo(() => {
    return requestRows.filter((row) => {
      if (normalizeText(row.status) !== "pending") return false;

      if (isLdSiteAdmin) {
        return normalizeText(row.userLocation) === normalizeText(currentUserLocation);
      }
      ...
      return normalizeText(row.userEmail) === normalizeText(currentUserEmail);
    });
  }, [requestRows, isLdSiteAdmin, isItHelpDesk, currentUserLocation, currentUserEmail]);

  // ---- Filtered rows for My Requests tab ----
  // Toggle ON  -> role-based "pending for my action" bucket (myPendingBucketRows):
  //                 L&D Site Admin: pending requests raised from their assigned Location
  //                 IT HelpDesk:    pending requests assigned to them (HelpdeskEmail)
  // Toggle OFF -> every request the current user personally raised (UserEmail),
  //                 regardless of status. Super Admin always sees everything and
  //                 never gets the toggle (hidden in the UI further below).
  ...
  const filteredRequestRows = useMemo(() => {
    ...
    } else if (myPendingOnly) {
      baseRows = myPendingBucketRows;
    } else {
    ...
    return baseRows.filter((row) => {
      ...
      return matchesHelpdeskContact && matchesStatus;
    });
  }, [
    requestRows,
    myPendingBucketRows,
  ...
  // L&D Site Admin can action requests raised from their assigned location.
  ...
  const handleMyPendingToggle = (_event: React.ChangeEvent<HTMLInputElement>, checked: boolean) => {
    setMyPendingOnly(checked);
    setPage(0);
  };
  ...
  const requestColumns: GridColDef[] = useMemo(() => {
    ...
    if (!isElevatedUser) {
      cols.push({
        ...
        renderCell: (params: GridRenderCellParams<RequestRow>) =>
          isAssignedToMe(params.row) && normalizeText(params.row.status) === "pending" ? (
            <Button size="small" variant="outlined" onClick={() => openActionDialog(params.row)}>
              Action
            </Button>
          ) : null,
      });
      ...
      <div className={styles.requestsToolbar}>
        ...
        {!isElevatedUser && (
          <FormControlLabel
            ...
            control={
              <Switch
                checked={myPendingOnly}
                onChange={handleMyPendingToggle}
                size="small"
                color="success"
              />
            }
