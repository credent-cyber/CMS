...
//   );
  ...
  // current user's *approver* bucket:
  //   - L&D Site Admin -> pending requests raised from their assigned location
  //   - IT HelpDesk     -> pending requests assigned to their email (HelpdeskEmail)
  // A regular employee (neither role) is not an approver, so the toggle
  // should never surface their own raised requests here -- it returns empty
  // for them. Super Admin never sees this toggle at all (handled in the UI).
