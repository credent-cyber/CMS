...
//   );
  ...
  const locationOptions = useMemo(() => {
    const set = new Set<string>();
    requestRows.forEach((row) => {
      if (row.userLocation) set.add(row.userLocation);
    });
    return Array.from(set).sort((a, b) => a.localeCompare(b));
  }, [requestRows]);
  ...
  const filteredRequestRows = useMemo(() => {
    return requestRows.filter((row) => {
      ...
      const matchesLocation =
        locationFilter === "All" || normalizeText(row.userLocation) === normalizeText(locationFilter);

      const matchesStatus =
        statusFilter === "All" || normalizeText(row.status) === normalizeText(statusFilter);

      const matchesRequestedBy =
        requestedByFilter === "All" || normalizeText(row.userName) === normalizeText(requestedByFilter);

      return matchesHelpdeskContact && matchesLocation && matchesStatus && matchesRequestedBy;
    });
  }, [requestRows, helpdeskContactFilter, locationFilter, statusFilter, requestedByFilter]);
  ...
  const handleRequestedByFilterChange = (event: SelectChangeEvent) => {
  ...
  const renderMyRequestsTab = () => (
    <div className={styles.requestsTabWrap}>
      <div className={styles.requestsToolbar}>
        ...
        <FormControl size="small" className={styles.statusSelect}>
          <Select value={locationFilter} onChange={handleLocationFilterChange} displayEmpty>
        ...
        <FormControl size="small" className={styles.statusSelect}>
          <Select
            ...
            onChange={handleRequestedByFilterChange}
            displayEmpty
          >
            <MenuItem value="All">Requested By</MenuItem>
            {requestedByOptions.map((opt) => (
              <MenuItem key={opt} value={opt}>
                {opt}
              </MenuItem>
            ))}
